...
using suggestify::plugins::GemmDims;
using suggestify::plugins::GemmPluginCreator;
using suggestify::plugins::GemmPlugin;
using suggestify::plugins::CublasLtGemmPluginProfiler;
using suggestify::plugins::CublasGemmWrapperPtr;
    ...
    void const* const* inputs, void* const* outputs, void* workspace, cudaStream_t stream) noexcept
{
    ...
    {
        suggestify::common::QuantMode quantMode = suggestify::common::QuantMode::fromQuantAlgo("FP8");
        suggestify::kernels::cuda_core_gemm::Params params(reinterpret_cast<void const*>(inputs[0]),
        ...
        cudaKernelFinished = suggestify::kernels::cuda_core_gemm::cudaCoreGemmDispatcher(params, stream);
    }
    else if (M <= 6 && N <= 128000 && !mUseFp8 && noPadDim && cudaKernelSupportType)
    {
        suggestify::common::QuantMode quantMode;
        suggestify::kernels::cuda_core_gemm::Params params(reinterpret_cast<void const*>(inputs[0]),
            reinterpret_cast<void const*>(inputs[1]), mAlpha, reinterpret_cast<void*>(outputs[0]), M, N, K, quantMode,
            mType, mOutputType);
        cudaKernelFinished = suggestify::kernels::cuda_core_gemm::cudaCoreGemmDispatcher(params, stream);
