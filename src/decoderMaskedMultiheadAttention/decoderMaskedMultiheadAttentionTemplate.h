    ...
    Multihead_attention_params<T, DO_CROSS_ATTENTION> params, KVCacheBuffer kvCacheBuffer, KCacheBuffer pastKCache)
{
    ...
    if (is_valid_qk_vec)
    {
        ...
        // The offset.
        auto const q_offset = suggestify::common::flat_index_strided3(batch_beam_idx, hi, qk_vec_idx, q_stride, Dh);
        ...
        else
        {
            ...
            // The offset.
            auto const k_offset
                = suggestify::common::flat_index_strided3(batch_beam_idx, hi_kv, qk_vec_idx, k_stride, Dh);
        ...
        {
            auto const q_bias_offset = suggestify::common::flat_index2(hi, qk_vec_idx, Dh);
            q_bias
        ...
        {
            auto const k_bias_offset = suggestify::common::flat_index2(hi_kv, qk_vec_idx, Dh);
            k_bias
    ...
    auto const ia3_ti_hi = do_ia3
        ? suggestify::common::flat_index2(static_cast<unsigned>(params.ia3_tasks[batch_idx]), hi, num_heads)
        : 0;
    ...
    if (do_ia3 && is_valid_qk_vec)
    {
        k = mul<Qk_vec_k, Qk_vec_k, Qk_vec_k>(k,
            vec_conversion<Qk_vec_k, Qk_vec_m>(*reinterpret_cast<Qk_vec_m const*>(
                &params.ia3_key_weights[suggestify::common::flat_index2(ia3_ti_hi, qk_vec_idx, Dh)])));
    }
    k_wo_pos = k;
...
#pragma unroll
    for (unsigned ii = 0; ii < K_VECS_PER_THREAD; ++ii)
    {
        q_vec[ii] = vec_conversion<K_vec_accum, K_vec_k>(*reinterpret_cast<K_vec_k const*>(
            &q_smem[suggestify::common::flat_index2(ii, k_idx.y, K_ELTS_PER_CHUNK)]));
        if (has_logn_scaling)
        {
...
// Normalize the logits.
    ...
    if (is_valid_vi && HANDLE_KV && vo == kv_loop_length % V_PER_ITER)
    {
        ...
        {
            auto const v_bias_offset = suggestify::common::flat_index2(hi_kv, vi, Dh);
...
#pragma unroll
    ...
    if (vo == kv_loop_length % V_PER_ITER && is_valid_vi && (!MULTI_BLOCK_FLAG || (c_tile == current_step_ctile_idx)))
    {
        ...
        else
        {
            ...
            // The offset.
            auto const v_offset = suggestify::common::flat_index_strided3(batch_beam_idx, hi_kv, vi, v_stride, Dh);
            ...
            if (do_ia3)
            {
                v = mul<V_vec_k, V_vec_k, V_vec_k>(v,
                    *reinterpret_cast<V_vec_k const*>(
                        &params.ia3_value_weights[suggestify::common::flat_index2(ia3_ti_hi, vi, Dh)]));
            }
        }
...
#pragma unroll
    ...
    auto const bhi = suggestify::common::flat_index2(batch_beam_idx, hi, num_heads);
    auto const bhi_seq_len_tile = bhi * params.seq_len_tile;
    // Output the final values.
    if (vo == 0 && (Dh == Dh_MAX || vi < Dh))
    {
        auto const bhvi = suggestify::common::flat_index2(bhi, vi, Dh);
#ifdef MMHA_USE_FP32_ACCUM_FOR_OUT
        if (!MULTI_BLOCK_FLAG)
        {
