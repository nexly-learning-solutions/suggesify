    ...
    XQAParams const& params, KVCacheBuffer kv_cache_buffer)
{
    ...
    workspace = suggestify::common::nextWorkspacePtrWithAlignment(
    ...
    workspace = suggestify::common::nextWorkspacePtrWithAlignment(workspace, cu_seqlens_size);
    launchParams.rotary_inv_freq_buf = reinterpret_cast<float*>(workspace);
    auto const multi_block_workspace_alignment = suggestify::common::roundUp(
...
template <typename T>
std::optional<T> getGlobalVar(std::shared_ptr<suggestify::common::CUDADriverWrapper> const& driver, CUmodule hmod,
