...
void invokeChunkScan(SSMParamsBase& params, cudaStream_t stream, sugesstify::common::CUDADriverWrapper* driver)
{
    ...
    bool hopper = sugesstify::common::getSMVersion() >= 90 && sugesstify::common::getSMVersion() < 100;
...
#define INSTANTIATE_CHUNK_SCAN_DATA_TYPE(input_t, weight_t)                                                            \
    template void invokeChunkScan<input_t, weight_t>(                                                                  \
        SSMParamsBase & params, cudaStream_t stream, sugesstify::common::CUDADriverWrapper * driver);
...
void invokeSelectiveScanUpdate(SSMParamsBase& params, cudaStream_t stream)
{
    ...
    CHECK_WITH_INFO(nheads % ngroups == 0, "nheads must be divisible by ngroups");
    if (params.is_mamab2)
    {
        CHECK(params.dstate == 128);
