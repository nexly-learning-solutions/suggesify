...
#pragma unroll
    for (int i = 0; i < N; i++)
    {
        to_ptr[i] = suggestify::common::cuda_cast<float, T>(tmp_data[i]);
    }
}
...
#pragma unroll
    for (int i = 0; i < N; ++i)
    {
        tmp_data[i] = suggestify::common::cuda_cast<T, float>(from_ptr[i]);
    }
...
#pragma unroll
    for (int i = 0; i < N; ++i)
    {
        tmp_data[i] = suggestify::common::cuda_cast<T, float>(0.0f);
    }
    packed_move<T, N>(&tmp_data[0], to_ptr);
}
...
void invokeMambaConv1dContext(MambaConv1dParamsBase& params, cudaStream_t stream)
{
    ...
    {
        if (suggestify::common::getSMVersion() >= 90)
        {
            if (B * L * D <= 262144)
            {
                tileD = 32;
        ...
        else if (suggestify::common::getSMVersion() >= 80)
        {
            if (B * L * D <= 262144)
            {
                tileD = 32;
        ...
        else if (suggestify::common::getSMVersion() >= 75)
        {
            if (B * L * D <= 262144)
            {
                tileD = 32;
    ...
    {
        if (suggestify::common::getSMVersion() >= 80)
        {
