...
using ::suggestify::kernels::XQAKernelRuntimeHashKey;
using ::suggestify::kernels::XQAParams;
using ::suggestify::kernels::XQAKernelMetaInfo;
...
DecoderXQAImplJIT::DecoderXQAImplJIT(DecoderXQARunner* runner)
    ...
    , mForceXQA(suggestify::common::forceXQAKernels())
    , mSM(suggestify::common::getSMVersion())
{
}
    ...
    int multiprocessor_count, cudaStream_t const& stream)
{
    ...
    if (needOutputCvt)
    {
        suggestify::kernels::invokeConversion<__nv_fp8_e4m3, T>(static_cast<__nv_fp8_e4m3*>(xqaParams.output),
