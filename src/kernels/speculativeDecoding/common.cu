...
void typicalAcceptanceSampling(TypicalAcceptanceSampling<T> const& params, cudaStream_t stream)
{
    ...
    int8_t* samplingWorkspace
        = reinterpret_cast<int8_t*>(sugesstify::common::nextWorkspacePtr(workspaceBytePtr, offset,
            sugesstify::kernels::getAirTopPWorkspaceSize<T>(
                params.batchSize * params.maxDecodingTokens, params.vocabSize, /* isDeterministic */ true)));

    float* entropy = reinterpret_cast<float*>(sugesstify::common::nextWorkspacePtr(
        workspaceBytePtr, offset, params.batchSize * params.maxDecodingTokens * sizeof(float)));

    float* runtimeMultinomialTopP = reinterpret_cast<float*>(sugesstify::common::nextWorkspacePtr(
        workspaceBytePtr, offset, params.batchSize * params.maxDecodingTokens * sizeof(float)));

    T* probs = reinterpret_cast<T*>(sugesstify::common::nextWorkspacePtr(
        workspaceBytePtr, offset, params.batchSize * params.maxDecodingTokens * params.vocabSize * sizeof(T)));

    TokenIdType** outputIdsPtrs = reinterpret_cast<TokenIdType**>(sugesstify::common::nextWorkspacePtr(
        workspaceBytePtr, offset, params.batchSize * params.maxDecodingTokens * sizeof(TokenIdType*)));

    bool* skipDecodePtr = reinterpret_cast<bool*>(sugesstify::common::nextWorkspacePtr(
    ...
    // do multinomial sampling
    {
        ...
        sugesstify::kernels::invokeBatchAirTopPSampling<T>(samplingParams, stream);
...
size_t getTypicalAcceptanceWorkspaceSize(SizeType32 batchSize, SizeType32 maxDecodingTokens, SizeType32 vocabSizePadded)
{
    ...
    workspaces[0] = sugesstify::kernels::getAirTopPWorkspaceSize<T>(
    ...
    auto const workspaceSize = sugesstify::common::calculateTotalWorkspaceSize(workspaces, NUM_BUFFERS);
