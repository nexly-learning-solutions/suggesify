...
size_t HopperGroupedGemmInput::workspaceSize(int num_experts)
{
    ...
    return suggestify::common::calculateTotalWorkspaceSize(buffers.data(), buffers.size());
}
    ...
    int8_t* start_ptr, int num_experts, void* gemm_workspace, size_t gemm_workspace_size)
{
    ...
    CHECK_WITH_INFO(pointers.size() == buffers.size(), "Mismatching workspace size and number of buffers");
    for (int i = 0; i < buffers.size(); i++)
    {
        pointers[i] = start_ptr;
        start_ptr = suggestify::common::nextWorkspacePtr(start_ptr, buffers[i]);
    }
