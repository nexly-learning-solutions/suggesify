...
void invokeMaskedSoftmax(MaskedSoftmaxParam<T, T_IN>& param, cudaStream_t stream)
{
    ...
    if (block.x > 32768)
    {
        CHECK(false); // Not implemented - it's not clear we want to use the unfused kernel in that case.
    }
    ...
    int const int8_mode, cudaStream_t stream)
{
    ...
    CHECK_WITH_INFO(kv_head_num != 0 || (k_buf == nullptr && v_buf == nullptr),
    ...
    {
        CHECK_WITH_INFO(int8_mode != 2, "w8a8 not yet implemented with RoPE"); // TODO
